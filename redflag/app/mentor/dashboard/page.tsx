...
import { studentNeedingAttention, studentsData } from "@/lib/studentData";
import Link from 'next/link';
import { Search, ArrowRight, Upload, BarChart, Mail, ChevronsLeft, ChevronsRight, Home, User, Users, Calendar, LogOut } from "lucide-react";
import {
  Table,
  TableBody,
  TableCaption,
...
const Scheduling = () => {
            ...
            <div className="flex items-center gap-2">
              {['All', 'At Risk', 'Improving', 'On Track'].map((status) => (
                <Button
                  ...
                  onClick={() => status==='On Track' ? setActiveFilter('good') : setActiveFilter(status)}
                >
                  {status}
                </Button>
              ))}
  ...
  const AlertRules = () => {
    return <div className="p-6">Alert rules configuration will appear here</div>;
  }
                    ...
                    {menuItems.map((item) => (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-6 py-3 hover:bg-gray-100 transition-colors duration-200 ${
                                ...
                                activeTab === item.id ? "bg-gray-100 font-semibold text-gray-900" : "text-gray-600"
                            } ${
